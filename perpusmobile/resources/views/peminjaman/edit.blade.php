...
<div class="bg-white shadow-md rounded-lg p-6">
    <h2 class="text-2xl font-bold mb-4">Edit Peminjaman #{{ $peminjaman->id }}</h2>
    <form action="{{ route('peminjaman.update', $peminjaman->id) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="user_id" class="block text-gray-700 text-sm font-bold mb-2">Peminjam (Anggota):</label>
            <select name="user_id" id="user_id" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('user_id') border-red-500 @enderror">
                ...
                <option value="{{ $user->id }}" {{ old('user_id', $peminjaman->user_id) == $user->id ? 'selected' : '' }}>
                    {{ $user->nama }} {{-- <-- UBAH DARI 'user->name' MENJADI 'user->nama' --}}
                </option>
                @endforeach
            </select>
            @error('user_id')
        ...
        <div class="mb-4">
            ...
            <input type="date" name="tanggal_pinjam" id="tanggal_pinjam" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tanggal_pinjam') border-red-500 @enderror" value="{{ old('tanggal_pinjam', \Carbon\Carbon::parse($peminjaman->tanggal_pinjam)->format('Y-m-d')) }}">
        ...
        <div class="mb-4">
            <label for="tanggal_kembali" class="block text-gray-700 text-sm font-bold mb-2">Tanggal Kembali (Estimasi):</label>
            <input type="date" name="tanggal_kembali" id="tanggal_kembali" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tanggal_kembali') border-red-500 @enderror" value="{{ old('tanggal_kembali', \Carbon\Carbon::parse($peminjaman->tanggal_kembali)->format('Y-m-d')) }}">
        ...
        <div class="mb-4">
            ...
            <select name="status" id="status" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('status') border-red-500 @enderror">
                ...
                <option value="dibatalkan" {{ old('status', $peminjaman->status) == 'dibatalkan' ? 'selected' : '' }}>Dibatalkan</option>
            </select>
            @error('status')
            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('peminjaman.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                Batal
            </a>
        </div>
    </form>
