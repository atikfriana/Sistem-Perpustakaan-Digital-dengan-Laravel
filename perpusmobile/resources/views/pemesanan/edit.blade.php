...
<div class="bg-white shadow-md rounded-lg p-6">
    <h2 class="text-2xl font-bold mb-4">Edit Pemesanan #{{ $pemesanan->id }}</h2>
    <form action="{{ route('pemesanan.update', $pemesanan->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="buku_id" id="buku_id" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('buku_id') border-red-500 @enderror">
                ...
                <option value="{{ $buku->id }}" {{ old('buku_id', $pemesanan->buku_id) == $buku->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="tanggal_pesan" id="tanggal_pesan" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tanggal_pesan') border-red-500 @enderror" value="{{ old('tanggal_pesan', \Carbon\Carbon::parse($pemesanan->tanggal_pesan)->format('Y-m-d')) }}">
            @error('tanggal_pesan')
            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        </div>
        ...
        <option value="siap_ambil" {{ old('status', $pemesanan->status) == 'siap_ambil' ? 'selected' : '' }}>Siap Ambil</option>
